	...
	get activities(): Activities {
		if (!this._activities) {
			...
			for (let i = 0, len = this.values.length; i < len; i++) {
				...
				if (value) {
					this._activities.set(this.keyDayParser(value.date), {
						count     : value.count,
	...
	get firstFullWeekOfMonths(): Month[] {
		if (!this._firstFullWeekOfMonths) {
			...
			for (let index = 1, len = cal.length; index < len; index++) {
				const lastWeekArr = cal[index - 1];
				const currentWeekArr = cal[index];
				if (lastWeekArr && lastWeekArr[0] && currentWeekArr && currentWeekArr[0]) {
					const lastWeek = lastWeekArr[0].date;
					const currentWeek = currentWeekArr[0].date;
