...
import { Heatmap } from '@/components/calendar-heatmap/Heatmap.ts'
import type {
  CalendarItem,
  Locale,
  Month,
  TooltipFormatter,
  Value,
} from '@/components/calendar-heatmap/Heatmap.ts'
import tippy, {
  createSingleton,
  ...
  setup(props) {
    ...
    watch(
      ...
      (v) => {
        if (v) {
          width.value =
            LEFT_SECTION_WIDTH + SQUARE_SIZE * Heatmap.DAYS_IN_WEEK + RIGHT_SECTION_WIDTH
          height.value =
          ...
          width.value =
            LEFT_SECTION_WIDTH + SQUARE_SIZE * heatmap.value.weekCount + SQUARE_BORDER_SIZE
    ...
    watch([values, tooltipUnit, tooltipFormatter, noDataText, max, rangeColor], () => {
    ...
    function initTippyLazy(e: MouseEvent) {
      if (
        ...
        if (!isNaN(weekIndex) && !isNaN(dayIndex)) {
          const week = heatmap.value.calendar[weekIndex]
          const day = week ? week[dayIndex] : undefined
          if (day) {
            ...
            if (tooltip) {
              ...
              if (instance) {
                ...
                tippyInstances.set(
                  ...
                  tippy(e.target as HTMLElement, { content: tooltip } as Partial<Props>),
                )
                tippySingleton.setInstances(Array.from(tippyInstances.values()))
              }
            }
