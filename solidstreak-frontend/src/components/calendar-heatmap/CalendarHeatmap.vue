...
<script lang="ts">
	import { defineComponent, nextTick, onBeforeUnmount, onMounted, type PropType, ref, toRef, toRefs, watch } from 'vue';
	import { Heatmap } from '@/components/calendar-heatmap/Heatmap.ts';
	import type { CalendarItem, Locale, Month, TooltipFormatter, Value } from '@/components/calendar-heatmap/Heatmap.ts';
						...
						if (day) {
							...
							if (tooltip) {
								...
								if (instance) {
									...
									tippyInstances.set(e.target as HTMLElement, tippy(e.target as HTMLElement, { content: tooltip } as Partial<Props>));
									tippySingleton.setInstances(Array.from(tippyInstances.values()));
								}
							}
						}
