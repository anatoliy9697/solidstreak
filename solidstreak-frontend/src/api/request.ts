...
type ApiRequest = PostUserInfoRequest | PostPutHabitRequest | DeleteHabitRequest | PostHabitCheckRequest;
...
type ApiResponse = PostUserInfoResponse | PutHabitResponse | DeleteHabitResponse | GetHabitsResponse | PostHabitCheckResponse;
...
export class ApiFetcher {
  ...
  async upsertUserInfo(user: User, tgChat: { tgId: number }): Promise<RequestResult> {
    ...
    if (this.username) { payload.meta = { username: this.username } as Metadata; }
  ...
  async fetchHabits(userId: number): Promise<RequestResult> {
    return await performRequest('get', `/api/v1/users/${userId}/habits?with_checks=true`, this.initData);
  }
  ...
  async putHabit(userId: number, habit: Habit): Promise<RequestResult> {
    const payload: PostPutHabitRequest = { data: habit };
    if (this.username) { payload.meta = { username: this.username } as Metadata; }
    return await performRequest('put', `/api/v1/users/${userId}/habits/${habit.id}`, this.initData, payload);
  }

  async deleteHabit(userId: number, habitId: number): Promise<RequestResult> {
    const payload: DeleteHabitRequest = {};
    if (this.username) { payload.meta = { username: this.username } as Metadata; }
    return await performRequest('delete', `/api/v1/users/${userId}/habits/${habitId}`, this.initData, payload);
  }

  async postHabitCheck(userId: number, habitId: number, habitCheck: HabitCheck): Promise<RequestResult> {
    const payload: PostHabitCheckRequest = { data: habitCheck };
    if (this.username) { payload.meta = { username: this.username } as Metadata; }
    return await performRequest('post', `/api/v1/users/${userId}/habits/${habitId}/checks`, this.initData, payload);
  }
